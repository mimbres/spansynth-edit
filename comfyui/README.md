...
- **MIDI keyboard:** expand **MIDI keyboard** in the editor, click **Connect MIDI**, allow access, and choose an input. Select a track and click the waveform for a start position, then **Record → Stop → Apply edits**. Record with velocity, sustain pedal, and GM monitoring while the selected audio plays. New notes replace same-pitch overlaps in that track. **Undo** restores the score before the take. **Snap recording** optionally uses the Snap grid. Connect the keyboard to the computer running your browser, even when generation runs on a server. Use Chrome or Edge over localhost or HTTPS.
