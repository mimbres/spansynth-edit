...
The first screen compares a Kraisler violin–piano recording with a prepared **16-step, CFG 2** result. The score highlights the violin phrase and shows the piano accompaniment in grey. Both players include both instruments and play only the **6.40–14.08 s passage** by default, stopping at its end. Choose **Full clip** to hear all 20.48 seconds. Listening needs no transcription or GPU generation. **Try this violin edit** loads the original and edited MIDI, selects the six violin notes, and sets the same region. Generate it again or change the notes. Enable **Auto region from note edits** to follow further changes. A new generation will vary from the prepared result.
